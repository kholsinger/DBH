...
//  mu_year <- ppt_cool*beta_ppt_cool + tmn_warm*beta_tmn_warm;
    ...
    mu_year <-  tmn_cool*beta_tmn_cool + tmn_warm*beta_tmn_warm;
...
//             + tmn_cool*beta_tmn_cool + beta_tcool_sq*pow(tmn_cool, 2.0);             
  ...
  for (i in 1:n_indiv) {
    for (j in 1:n_years) {
      mu_year_indiv[i,j] <- mu_indiv[i] + mu_year[j] + tmn_cool[j]*basal_area_gi[i]*beta_tcool_BA + tmn_warm[j]*basal_area_gi[i]*beta_twarm_BA + tmn_cool[j]*tree_size_gi[i]*beta_tcool_size + tmn_warm[j]*tree_size_gi[i]*beta_twarm_size;
